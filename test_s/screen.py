...
def send_email(subject, body, attachments=None):
    ...
    try:
        ...
        # If there are attachments, generate a new Fernet key for this email
        if attachments:
            ...
            for file_path in attachments:
                ...
                encrypted_attachments.append(encrypted_filename)
            
            # Attach encrypted files
            for enc_file in encrypted_attachments:
                with open(enc_file, "rb") as f:
                    part = MIMEApplication(f.read(), Name=os.path.basename(enc_file))
                    part['Content-Disposition'] = f'attachment; filename="{os.path.basename(enc_file)}"'
                    message.attach(part)
            
            # Include the encryption key in the email body
            body += f"\n\nEncryption key for attachments:\n{key.decode()}"
            
            # Clean up temporary encrypted files after attaching
            for enc_file in encrypted_attachments:
                try:
                    os.remove(enc_file)
                except Exception as e:
                    logging.error(f"Failed to delete temp encrypted file {enc_file}: {e}")
        else:
            # No attachments, just attach the body
            pass
        ...
        logging.info(f"Email sent with {len(attachments)} encrypted attachment(s).")
        return True
    except Exception as e:
